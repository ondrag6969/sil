...
def sync(srcdev, dsthost, dstdev=None, blocksize=1024 * 1024):
    ...
    cmd = ['ssh', '-p 23', dsthost, 'python', 'blocksync.py', 'server', dstdev, '-b', str(blocksize)]
    ...
    for i, l_block in enumerate(getblocks(f, blocksize)):
        ...
        if t1 - t_last > 1:
            ...
            eprint ("\rsame: %d, diff: %d, %d/%d, %5.1f MB/s" % (same_blocks, diff_blocks, same_blocks + diff_blocks, size / blocksize, rate),)
            t_last = t1

    print "\nsame: %d, diff: %d" % (same_blocks, diff_blocks)
...
if __name__ == "__main__":
    ...
    parser.add_option("-q", dest="quiet", action="store_true", help="potichu")
